<script>
  ...
  /**
   * Two-step CSV import flow for companies: upload → preview → confirm.
   * Network calls go to the SvelteKit form actions on accounts/+page.server.js,
   * which forward the multipart upload to Django.
   *
  ...
  /**
   * Per-row errors returned by the commit endpoint if the file changed
  ...
  function downloadErrorsList(errs) {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = 'companies-import-errors.csv';
    a.click();
    URL.revokeObjectURL(url);
  }
  ...
  async function submitCommit() {
    ...
    try {
      ...
      if (result.type === 'success' && result.data?.importCommit) {
        ...
        toast.success(`Imported ${n} compan${n === 1 ? 'y' : 'ies'}`);
      }
    } catch (err) {
...
<Dialog.Root bind:open onOpenChange={(v) => onOpenChange?.(v)}>
  <Dialog.Content class="sm:max-w-3xl">
    <Dialog.Header>
      ...
      <Dialog.Description>
        Upload a CSV of companies. We'll validate every row — including duplicate
    ...
    {#if step === 'select'}
      <div class="space-y-4">
        <button
          ...
          ondrop={onDrop}
          onclick={() => document.getElementById('account-import-file')?.click()}
        >
        ...
        <div class="rounded-lg border border-[var(--border-default)] bg-[var(--surface-muted)] p-3 text-xs">
          ...
          <p class="mt-1 text-[var(--text-secondary)]">
            Required header: <code class="rounded bg-[var(--surface-default)] px-1">name</code>.
            Optional: email, phone, website, industry, number_of_employees,
            annual_revenue, currency (3-letter code), address_line, city, state,
            postcode, country (2-letter code), description.
          </p>
          <p class="mt-1 text-[var(--text-secondary)]">
            Duplicates: a row whose company name matches an existing company in
            your organization — or an earlier row in the file — is flagged. Fix
            those rows before importing.
          </p>
          <button
            type="button"
    ...
    {:else if step === 'preview' && preview}
            ...
            <table class="w-full text-xs">
              <thead class="sticky top-0 bg-[var(--surface-muted)]">
                <tr>
                  ...
                  <th class="px-2 py-1 text-left font-medium">Website</th>
                  <th class="px-2 py-1 text-left font-medium">Industry</th>
                </tr>
              </thead>
              <tbody>
                {#each preview.valid.slice(0, 20) as row (row.row)}
                  <tr class="border-t border-[var(--border-default)]">
                    ...
                    <td class="max-w-[180px] truncate px-2 py-1">{row.name}</td>
                    <td class="max-w-[180px] truncate px-2 py-1 text-[var(--text-secondary)]">
                      {row.email || '—'}
                    </td>
                    <td class="max-w-[160px] truncate px-2 py-1 text-[var(--text-secondary)]">
                      {row.website || '—'}
                    </td>
      ...
      <Dialog.Footer>
        ...
        >
          {#if busy}<Loader2 class="mr-1 h-3.5 w-3.5 animate-spin" />{/if}
          Import {preview.summary.valid} compan{preview.summary.valid === 1 ? 'y' : 'ies'}
        </Button>
      </Dialog.Footer>
        ...
        <div class="flex items-center gap-3 rounded-lg border border-green-200 bg-green-50 p-4">
          ...
          <div>
            <p class="text-sm font-medium text-green-900">
              Imported {commitResult.created} compan{commitResult.created === 1 ? 'y' : 'ies'}
            </p>
            <p class="text-xs text-green-800">
              The companies list has been refreshed.
            </p>
          </div>
        </div>
      </div>
